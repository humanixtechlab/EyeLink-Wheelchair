...
Commercial eye-gaze/head-control wheelchair systems are specialized medical devices: a standard power wheelchair base alone typically runs ₹2,00,000–₹5,00,000 in India, and a dedicated eye-gaze or head-array control add-on (often imported, from companies like EyeTech or Quantum Rehab) can push the total well past ₹8,00,000–₹16,00,000. **EyeLink** does the same core job — hands-free wheelchair control — for around ₹2,500 (~$30), using a WeMos D1 Mini, an MPU-6050, two IR blink sensors, and ESP-NOW wireless. You tilt your head in a direction and confirm with a specific eye-blink pattern; the wheelchair base receives the command instantly over a direct peer-to-peer wireless link — no Wi-Fi, no internet, no app. It's a working proof-of-concept that life-changing assistive tech doesn't have to be expensive.
...
> 📷 Add a screenshot under each step below — see [`docs/IMAGE_SHOT_LIST.md`](IMAGE_SHOT_LIST.md#firmware--arduino-ide-screenshots-firmware_screenshots) for exact filenames. These are the screenshots people will actually compare their own Arduino IDE against, so they matter a lot for a smooth first build.
...
![Serial Monitor showing printed MAC address](images/firmware_screenshots/fw_01_mac_output.png)
...
![rxMAC line in TX_Glasses_FINAL.ino](images/firmware_screenshots/fw_02_rxmac_in_code.png)
...
![Calibration helper printing the #define block](images/firmware_screenshots/fw_03_calibration_output.png)
...
![Serial Monitor showing Waiting for TX](images/firmware_screenshots/fw_04_waiting_for_tx.png)
...
![Status LED solid when active](images/firmware_screenshots/fw_05_status_led.jpg)
